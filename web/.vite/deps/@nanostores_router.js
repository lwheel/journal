...
// node_modules/.pnpm/@nanostores+router@0.15.0_nanostores@0.10.3/node_modules/@nanostores/router/index.js
function createRouter(routes, opts = {}) {
  ...
  router.routes = Object.keys(routes).map((name) => {
    ...
    let pattern = value.replace(/[\s!#$()+,.:<=?[\\\]^{|}]/g, "\\$&").replace(/\/\\:(\w+)\\\?/g, "(?:/(?<$1>(?<=/)[^/]+))?").replace(/\/\\:(\w+)/g, "/(?<$1>[^/]+)");
    return [
      name,
      RegExp("^" + pattern + "$", "i"),
      null,
